...
const detectLanguage = (code: string): string => {
  switch (true) {
    ...
    case /{[^}]*}/.test(code) && !/(int|public|class|def)/.test(code):
      return "css";
    case /class\s+[A-Za-z]+\s+{/.test(code) &&
    ...
    case /<\?php/.test(code) ||
      /echo\s+['"]/.test(code) ||
      /function\s+[A-Za-z_]+\s*\(/.test(code) ||
      /<\?=/.test(code):
      return "php";
    case /(SELECT|INSERT|UPDATE|DELETE|WITH|JOIN|UNION|CREATE|ALTER|DROP|WHERE|GROUP\s+BY|ORDER\s+BY|LIMIT|OFFSET|HAVING)/i.test(
      code,
    ):
      return "sql";
    ...
    case /fn\s+main\s*\(\)/.test(code) && /println!\s*\(/.test(code):
      return "rust";
    case /(BEGIN|END;|DECLARE|EXCEPTION|LOOP|IF|THEN|ELSE|CURSOR|FETCH|EXIT)/i.test(
      code,
    ):
      return "plsql";
      ...
      <Card key={issue?.id} className="mb-4">
        ...
        <CardContent>
          <pre className="bg-muted p-2 rounded-md overflow-x-auto">
            <CodeBlock
              code={issue?.code}
              language={detectLanguage(issue?.code)}
            />
          </pre>
          {issue?.src && (
            <img
          ...
          <section className="px-4 break-words whitespace-pre-wrap overflow-hidden overflow-x-auto">
            {editingComment?.id === comment.id ? (
              ...
              <CodeBlock
                ...
                language={detectLanguage(comment?.description)}
              />
            )}
          </section>
          {comment.src && (
